...
4. **Operación a prueba de ausencias (esfuerzo: días, decisión del usuario).** Ejecutar la migración a secret manager del runbook; segundo humano con acceso de lectura a `/observabilidad` y al correo de equipo; plantilla de post-mortem de 5 líneas; branch protection server-side al pasar a Pro. 🟡 **Piezas de código/docs entregadas (2026-07-03); lo externo queda pendiente de tu acción.** Entregado: (a) **gestión de usuarios** — `GET/POST /api/users` + `PATCH /api/users/{id}` (admin, aislado por tenant, hash bcrypt nunca expuesto, guardas anti auto-bloqueo, desactivar corta la sesión viva vía S2) + `scripts/create_user.py` → habilita al 2.º operador (viewer de solo-lectura); (b) **plantilla de post-mortem** `docs/postmortem-template.md`; (c) **scaffolding de secret manager** `deploy/k8s/secret-manager/` (External Secrets Operator, provider-agnostic, materializa el mismo Secret `agente-rh-secrets`); (d) **`scripts/setup-branch-protection.sh`** (gh api, 5 checks requeridos). **Pendiente (requiere cuentas/decisiones externas):** cargar los secretos en un gestor real y aplicar el `ExternalSecret`; dar de alta y onboardear al 2.º humano; correr el script de branch protection tras pasar el repo a Pro o público.
