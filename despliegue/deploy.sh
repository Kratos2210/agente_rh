...
#
# Uso:  despliegue/deploy.sh <comando> [args]
#
...
#                        agente-rh-<env> (exige despliegue/k8s/secret.yaml).
...
cmd_validate() {
  ...
  for env in dev prod; do
    echo "== overlay $env =="
    "$KUBECTL" kustomize "$ROOT/despliegue/k8s/overlays/$env/" \
...
cmd_k8s_apply() {
  ...
  [ -f "$ROOT/despliegue/k8s/secret.yaml" ] || {
    echo "Falta despliegue/k8s/secret.yaml (copiá secret.example.yaml y completalo; NO lo commitees)"
    exit 1
  }
  # El overlay ya declara el Namespace; se aplica primero para que el secret tenga dónde ir.
  "$KUBECTL" apply -f "$ROOT/despliegue/k8s/overlays/$env/namespace.yaml"
  "$KUBECTL" apply -f "$ROOT/despliegue/k8s/secret.yaml" -n "$ns"
  "$KUBECTL" apply -k "$ROOT/despliegue/k8s/overlays/$env/"
