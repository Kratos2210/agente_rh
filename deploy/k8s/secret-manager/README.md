...
usando `deploy/k8s/secret.example.yaml` (Secret plano), y prod puede empezar plano y migrar
...
2. **Cargar los secretos en el gestor** con los MISMOS nombres que las env vars del backend
   (ver la lista en `externalsecret.example.yaml` y en `deploy/k8s/secret.example.yaml`).
